function display_entry_2(entry_object) {
...
var eventSource = new EventSource("/stream/entrypool")
mem = null
eventSource.onmessage = function(e) {
    ...
    mem = e.data
    formatted = e.data.replace(/'/g, '"')
    jsonobj = JSON.parse(formatted);
    targetContainer.innerHTML =`
      <h1 class="app-title">Entry Pool (${jsonobj.length} Entrie/s)</h1>
      ${jsonobj.map(display_entry_2).join("")}
    `;
};